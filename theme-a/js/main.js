jQuery(document).ready(function($){
	...
	$().prettyPhoto({ // fire it up
		...
		callback: function() {
			...
			// move to current section, because after hide/show, doc height changed and user ends up at very top (not desired if portfolio is not top section)
			var portfolio_section_id = $('.section-portfolio').attr('id');
			if (portfolio_section_id) {
				window.location.hash = ''; // first remove hash, in case it is same as current, because then position will not change
				window.location.hash = $('.section-portfolio').attr('id');
			}
	...
	$("body").imageLoader({
		...
		// Slide in effect
		var scrollable = $(document).height() > $(window).height() + 50; 
		if (!no_scrolling_effects && scrollable) { // if document height greater than viewport, scrolling can be done (+50 is so scrolling can be seen)
